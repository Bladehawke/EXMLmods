...
local modName = "SalvagedFrigateModuleRewardsEnhanced"
local description = "Improves Savaged Frigate Module rewards percentages, amounts, and sources. Fully configurable!"
local gameVersion = "5.58"
local modVersion = "1.3"
-- maintenance = author
...
local function round(x,n) --lua doesn't have proper rounding built in
	n = n or 0
	x = x * 10^n
...
--| Add New Rewards
--|=======================================================================================--
local FrigToken_RewardTableItem = string.format(
'<Property value="GcRewardTableItem"><Property name="PercentageChance" value="100" /><Property name="LabelID" value="" /><Property name="Reward" value="GcRewardSpecificProduct"><Property name="GcRewardSpecificProduct"><Property name="Default" value="GcDefaultMissionProductEnum"><Property name="DefaultProductType" value="None" /></Property><Property name="ID" value="FRIG_TOKEN" /><Property name="AmountMin" value="%s" /><Property name="AmountMax" value="%s" /><Property name="HideAmountInMessage" value="False" /><Property name="ForceSpecialMessage" value="False" /><Property name="HideInSeasonRewards" value="False" /><Property name="Silent" value="False" /><Property name="SeasonRewardListFormat" value="" /><Property name="RequiresTech" value="" /></Property></Property></Property>',
...
if _AddFreighterRewards then
	table.insert(RewardTable_EXML_CT,{ --Freighter Rescue. Awarded every time.
		["SKW"] = {"Id","FREIGHTER_SAVED","List","GcRewardTableItemList"},
		["PKW"] = "List",
	...
	table.insert(RewardTable_EXML_CT,{ --Freighter Destruction.  Not awarded every time, one in a pool of possible rewards.
		["SKW"] = {"Id","R_PIR_FREI","List","GcRewardTableItemList"},
		["PKW"] = "List",
...
if _AddPirateRewards then
	table.insert(RewardTable_EXML_CT,{ --Pirate Ship kills. Not awarded every time, one in a pool of possible rewards.
		["SKW"] = {"Id","PIRATELOOT","List","GcRewardTableItemList"},
		["PKW"] = "List",
		["ADD"] = string.gsub(FrigToken_RewardTableItem, 'Chance" value="100"', string.format('Chance" value="%s"',round(100*_PercentageChanceMult)))
	})
end
